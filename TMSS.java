...
public class TMSS {
	...
	public String validateDirection(String direction) throws IllegalArgumentException {
		if (!this.isValidDirection(direction)) {
			...
			for (int i = 0; i < this.validDirections.length; i++) {
				s.append(this.validDirections[i] + (i != this.validDirections.length - 1 ? ", " : ""));
			}
	...
	public String[] getValidDirections() {
		String[] result = new String[this.validDirections.length];
	...
	private int validateStringsCount(int minLength, int maxLength) {
	...
	private Object[] setInitialString(String initialString, boolean validate) {
	...
	public long getMaxProcessTime() {
		if (this.isSimulating) { // return in nanoseconds
			return (TMSS.NANOSECONDS_PER_MILLISECOND * this.maxProcessTime);
		}
	...
	public TMSS(Scanner in) throws IllegalArgumentException {
		...
		try {
			...
			this.lineNumber++;
			String[] s = line.split(" ");
			if (s.length != TMSS.LINE_1_NUM_ENTRIES || TMSS.countSpaces(line) != s.length - 1) {
			...
			this.lineNumber++;
			s = line.split(" ");
			if (s.length != tapeAlphabetSize - TMSS.NUM_SPECIAL_TAPE_CHARS || TMSS.countSpaces(line) != s.length - 1) {
			...
			this.lineNumber++;
			s = line.split(" ");
			if (s.length != TMSS.LINE_3_NUM_ENTRIES || TMSS.countSpaces(line) != s.length - 1) {
			...
			if (in.hasNextLine()) {
				...
				if (!line.isEmpty()) {
					s = line.split(" ");
					if (s.length > TMSS.COMMAND_LINE_MAX_NUM_ENTRIES || TMSS.countSpaces(line) != s.length - 1) {
			...
			while (in.hasNextLine()) {
				...
				this.lineNumber++;
				comments.append(in.hasNextLine() ? "\n" : "");
			}
	...
	public String[] putTransitions(int numTransitions, String[] transitions, boolean[] replace)
		...
		for (int i = 0; i < numTransitions; i++) {
			...
			this.lineNumber += this.isScanning ? 1 : 0;
			if (line == null) {
	...
	public Object[] validateTransition(String transition) throws IllegalArgumentException {
		...
		if (s.length != 5 || TMSS.countSpaces(transition) != s.length - 1) {
	...
	private static void illegalArg(String cause) {
